...
def is_cmd_not_found(stderr: str) -> bool:
    """Detect Windows 'command not found' across common locales.
    Matches EN and PT-BR variants and ignores ANSI sequences.
    """
    if not stderr:
        return False
    # remove ANSI
    s = re.sub(r"\x1b\[[0-9;]*m", "", stderr)
    s = s.lower()
    patterns = [
