...
class TestPyenvFeatureExec(TestPyenvBase):
    ...
    And we remove fake python.exe (empty file generated) to ensure sys.executable is found and used.
    ...
    def test_many_paths(self, setup):
        ...
        args = [pyenv, 'exec', 'python', '-c', "import os; print(os.environ['PATH'])"]
        result = subprocess.run(args, capture_output=True, encoding="utf8")
        # \x0c: generated by cls in cmd AutoRun
        stdout = result.stdout.strip("\r\n\x0c")
    ...
    def test_loop_shim_unavailable(self, setup):
        with TemporaryEnvironment({'_pyenv_exec': 'python3'}):
            pyenv = str(self.__class__.ctx.pyenv_path / "bin" / "pyenv.bat")
            args = [pyenv, 'exec', 'python3']
            result = subprocess.run(args, capture_output=True, encoding="utf8")
            stderr = result.stderr.strip("\r\n")
            if stderr != '':
                print(f'stderr: {stderr}')
            # \x0c: generated by cls in cmd AutoRun
            stdout = result.stdout.strip("\r\n\x0c")
            assert stdout == f"'python3' not found (shim exists but target missing in " \
                             f"'{self.__class__.ctx.pyenv_path}\\versions\\3.8.9')"
